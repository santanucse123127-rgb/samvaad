        ...
        <div className="flex flex-col h-full bg-transparent overflow-hidden">
            <AnimatePresence mode="wait">
                {activeSection === 'menu' ? (
                    ...
                    >
                        {menuItems.map(item => (
                            <button
                                ...
                                className="flex items-center justify-between p-4 rounded-2xl bg-[hsl(var(--sv-surface-2))] hover:bg-black/5 border border-black/5 transition-all group"
                            >
                                <div className="flex items-center gap-4">
                                    <div className={`w-10 h-10 rounded-xl bg-black/5 flex items-center justify-center ${item.color} group-hover:scale-110 transition-transform`}>
                                        <item.icon size={20} />
                                    </div>
                                    <span className="text-sm font-bold text-sv-text">{item.label}</span>
                                </div>
                                <ChevronRight size={18} className="text-sv-text-3" />
                            </button>
                        ))}
                    ...
                    >
                        {/* Sub-header */}
                        <div className="flex items-center gap-3 px-6 py-4 border-b border-[hsl(var(--sv-border))]">
                            <button
                                onClick={() => setActiveSection('menu')}
                                className="p-2 hover:bg-black/5 rounded-xl text-sv-text-3 hover:text-sv-text transition-all"
                            >
                                <ChevronRight size={20} className="rotate-180" />
                            </button>
                            <h3 className="text-sm font-black uppercase tracking-widest text-sv-text">
                        ...
                        <div className="flex-1 overflow-y-auto scrollbar-custom p-6">
                            ...
                            {activeSection === 'theme' && (
                                <div className="space-y-8">
                                    <div className="space-y-4">
                                        <p className="text-[10px] font-black uppercase tracking-widest text-sv-text-3 ml-1">Vibe Filter</p>
                                        <div className="grid grid-cols-2 gap-3">
                                            {vibes.map(v => (
                                                <button
                                                    key={v.id}
                                                    onClick={() => setActiveVibe(v.id)}
                                                    className={`p-4 rounded-3xl border-2 flex flex-col items-center gap-3 transition-all ${activeVibe === v.id ? 'border-sv-accent bg-sv-accent/10' : 'border-[hsl(var(--sv-border))] bg-[hsl(var(--sv-surface-2))] hover:bg-black/5'}`}
                                                >
                                                    ...
                                                    <span className="text-[11px] font-black uppercase tracking-tighter text-sv-text">{v.name}</span>
                                                </button>
                                            ))}
                                        </div>
                                    </div>
                                    <div className="space-y-4">
                                        <p className="text-[10px] font-black uppercase tracking-widest text-sv-text-3 ml-1">Wallpaper</p>
                            ...
                            {activeSection === 'privacy' && (
                                <div className="space-y-6">
                                    <div className="flex items-center justify-between p-4 rounded-2xl bg-[hsl(var(--sv-surface-2))] border border-[hsl(var(--sv-border))]">
                                        <div>
                                            <p className="text-sm font-bold text-sv-text">Read Receipts</p>
                                            <p className="text-[11px] text-sv-text-3">Don't show when you read messages.</p>
                                        </div>
                                        <button
                                            onClick={() => toggleSetting('readReceipts')}
                                            className={`w-12 h-6 rounded-full relative transition-all ${settings.readReceipts ? 'bg-sv-accent' : 'bg-black/10'}`}
                                        >
                                    ...
                                    <div className="flex items-center justify-between p-4 rounded-2xl bg-[hsl(var(--sv-surface-2))] border border-[hsl(var(--sv-border))]">
                                        <div>
                                            <p className="text-sm font-bold text-sv-text">Notifications</p>
                                            <p className="text-[11px] text-sv-text-3">Push alerts for new messages.</p>
                                        </div>
                                        <button
                                            onClick={() => toggleSetting('notifications')}
                                            className={`w-12 h-6 rounded-full relative transition-all ${settings.notifications ? 'bg-sv-accent' : 'bg-black/10'}`}
                                        >
                            ...
                            {activeSection === 'security' && (
                                <div className="space-y-6">
                                    <div className="p-6 rounded-3xl bg-sv-accent/5 border border-sv-accent/10 flex flex-col items-center text-center gap-4">
                                        <div className="w-16 h-16 rounded-3xl bg-sv-accent/10 flex items-center justify-center text-sv-accent">
                                            <Lock size={32} />
                                        </div>
                                        <h4 className="text-lg font-black italic tracking-tighter text-sv-accent">Secure Messaging</h4>
                                        <p className="text-xs text-sv-text-3">Your account is secured with End-to-End Encryption.</p>
                                    </div>

                                    <div className="space-y-4">
                                        <p className="text-[10px] font-black uppercase tracking-widest text-sv-text-3 ml-1">App Lock</p>
                                        <div className="flex items-center justify-between p-4 rounded-2xl bg-[hsl(var(--sv-surface-2))] border border-[hsl(var(--sv-border))] transition-all">
                                            <div className="flex items-center gap-3">
                                                <Key size={18} className="text-sv-text-2" />
                                                <span className="text-sm font-bold text-sv-text">App Lock (PIN)</span>
                                            </div>
                                            <button
                                                onClick={() => {
                                                ...
                                                className={`w-12 h-6 rounded-full relative transition-all ${user?.appLock?.enabled ? 'bg-sv-accent' : 'bg-black/10'}`}
                                            >
                                    ...
                                    <button className="w-full py-4 rounded-2xl bg-[hsl(var(--sv-surface-2))] text-[10px] font-black uppercase tracking-widest border border-[hsl(var(--sv-border))] hover:bg-black/5 text-sv-text transition-all flex items-center justify-center gap-3">
                                ...
                                <div className="space-y-4">
                                    {loadingDevices ? (
                                        ...
                                        linkedDevices.map(device => (
                                            <div key={device._id} className="p-4 rounded-2xl bg-[hsl(var(--sv-surface-2))] border border-[hsl(var(--sv-border))] flex items-center justify-between">
                                                <div className="flex items-center gap-4">
                                                    {device.deviceType === 'Mobile' ? <Smartphone size={18} className="text-sv-text-3" /> : <Monitor size={18} className="text-sv-text-3" />}
                                                    <div>
                                                        <p className="text-xs font-bold text-sv-text">{device.deviceName}</p>
                                                        <p className="text-[10px] text-sv-text-3">{device.os}</p>
                                                    </div>
                                                </div>
                                        ...
                                        <p className="text-center text-xs text-sv-text-3 py-20">No other active sessions.</p>
                                    )}
                                </div>
                            )}
                        </div>
